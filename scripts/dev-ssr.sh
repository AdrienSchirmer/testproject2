...
cleanup_container_dev_processes() {
    pkill -f "php artisan queue:listen --tries=1" >/dev/null 2>&1 || true
    pkill -f "php artisan pail --timeout=0" >/dev/null 2>&1 || true
    pkill -f "php artisan inertia:start-ssr" >/dev/null 2>&1 || true
...
if inside_sail; then
    ...
    exec npx concurrently -c "$colors" \
        ...
        --names=queue,logs,ssr --kill-others
fi

if uses_sail; then
    ./vendor/bin/sail up -d >/dev/null
    prepare_host_sail_frontend
    ./vendor/bin/sail bash -lc 'pkill -f "php artisan queue:listen --tries=1" >/dev/null 2>&1 || true; pkill -f "php artisan pail --timeout=0" >/dev/null 2>&1 || true; pkill -f "php artisan inertia:start-ssr" >/dev/null 2>&1 || true; pkill -f "vite" >/dev/null 2>&1 || true'
